...
<div class="row">
    <div class="col-md-12 mb-1">
        <strong>Permisos de :: {{$group->getName()}}</strong>
    </div>
</div>
<ul class="file-tree">
                    ...
                    @if($pvt2->cs_group == $pivot->cs_group)
                        <li>
                            <input type="checkbox"
                                   value="{{$pvt2->getId()}}"
                                   name="optCheckboxes"
                                   @foreach($group->permissions as $pvt3)
...
<div class="form-actions center">
    <button type="button"
            onclick="saveChangeTreeCheckbox('{{ route('sync.groups.permisions',$group->getId()) }}','optCheckboxes','{{$group->getId()}}');"
            class="btn btn-green">
        <i class="icon-save"></i>
        Guardar</button>
</div>
